...
architecture behavior of min_8_tb is
    ...
    signal num_min_tb : std_logic_vector(7 downto 0);
...
begin
    ...
    stim_proc: process
        variable expected_min : std_logic_vector(7 downto 0);
        ...
        begin
            ...
            -- Expected: min of a and b (unsigned compare)
            -- NOTE: Your DUT outputs num_b when a == b (because l_sel='0'),
            -- but since values are equal, this still matches expected.
            ...
            if num_min_tb /= expected_min then
                assert false
                    report tag & " FAILED | " &
                           ...
                           "Expected MIN=" & slv_u8_to_dec_str(expected_min) &
                           " (0x" & slv_u8_to_hex_str(expected_min) & ", " &
                           slv_u8_to_bin_str(expected_min) & ") | " &
                           "Got MIN=" & slv_u8_to_dec_str(num_min_tb) &
                           " (0x" & slv_u8_to_hex_str(num_min_tb) & ", " &
                           slv_u8_to_bin_str(num_min_tb) & ")"
                    severity error;
            end if;
        end procedure;
    ...
    begin
        ...
        apply_and_check(x"00", x"01", "T2 a smaller");
        apply_and_check(x"01", x"00", "T3 b smaller");
        apply_and_check(x"7F", x"80", "T4 boundary a smaller");
        apply_and_check(x"80", x"7F", "T5 boundary b smaller");
        apply_and_check(x"FF", x"00", "T6 b smaller");
        apply_and_check(x"00", x"FF", "T7 a smaller");
        ...
        report "All min_8 tests passed successfully!" severity failure;
        wait;
    end process;
