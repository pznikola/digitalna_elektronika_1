...
% Izvor DOCX: blok 42
U opštem slučaju, za broj D predstavljen u dva brojna sistema sa osnovom \emph{r} i \emph{p} važi:
...
% Izvor DOCX: blok 44
Ukoliko su poznate cifre broja u brojnom sistemu sa osnovom \emph{r}, cifre broja u brojnom sistemu sa osnovom \emph{p} dobijamo razvojem sume sa leve strane jednačine \eqref{eq:1.1.1.3} i izvođenjem operacija sabiranja i stepenovanja u brojnom sistemu sa osnovom \emph{p}. Zbog toga što je operacije sabiranja i stepenovanja najlakše izvesti u brojnom sistemu sa osnovom 10, formula \eqref{eq:1.1.1.3} se može iskoristiti za transformaciju broja iz brojnog sistema sa osnovom \emph{r} u dekadni brojni sistem i to na sledeći način:
...
\item ukoliko je \emph{k}\textsubscript{1} različito od 0, u sledećem koraku se \emph{k}\textsubscript{1} deli sa \emph{r}, čime se dobija količnik \emph{k}\textsubscript{2} i ostatak \emph{o}\textsubscript{2}
...
D=(o_io_{i-1}\dots o_1.s_1s_2\dots s_j)_r
\end{equation}
...
% Izvor DOCX: blok 70
U slučaju da digitalni sistem koristi ovaj način za predstavu označenih brojeva, bit najveće težine predstavlja znak dok preostali biti predstavljaju apsolutnu vrednost broja. Dakle, posmatrani broj \textbf{D}, koji je predstavljen na \emph{n} bita (\emph{D\textsubscript{n-1}D\textsubscript{n-2}\ldots{}D\textsubscript{1}D\textsubscript{0}}), se interpretira kao pozitivan ukoliko bit najveće težine \emph{D\textsubscript{n-1}} ima vrednost 0 dok se u suprotnom broj D interpretira kao negativan. Preostalih \emph{n-1} bita se koriste za predstavu apsolutne vrednosti broja D. Dakle, skup vrednosti koje je moguće predstaviti ukoliko digitalni sistem koristi predstavu „Znak i apsolutna vrednost“ je:
...
% Izvor DOCX: blok 74
Za nenegativnu apsolutnu vrednost $D$ i $n$ cifara u osnovi $r$, kodna reč negativnog broja u komplementu maksimalne vrednosti (KMV) dobija se kao:
...
% Izvor DOCX: blok 79
Jednačina \eqref{eq:1.2.2.1} se može tumačiti i na sledeći način: Ukoliko je negativna vrednost -D predstavljena na nivou cifara jednaka -d\textsubscript{n-1}d\textsubscript{n-2}\ldots{}d\textsubscript{1}d\textsubscript{0} i ako je maksimalna vrednost M predstavljena na nivou cifara jednaka mm\ldots{}mm tada za cifre negativnog broja D važi:
...
% Izvor DOCX: blok 81
Dakle, na osnovu \eqref{eq:1.2.2.3} zaključujemo da se u slučaju predstave negativnog broja D u komplementu maksimalne vrednosti vrednost svake cifre predstave broja dobija oduzimanjem odgovarajuće cifre negativnog broja od maksimalne cifre koju je moguće prikazati u datoj osnovi.
...
% Izvor DOCX: blok 90
gde M predstavlja maksimalnu vrednost broja koji je moguće predstaviti u datoj osnovi sa datim brojem cifara i definisana je relacijom \eqref{eq:1.2.2.2}. Dakle, na osnovu \eqref{eq:1.2.3.1}, predstavu apsolutne vrednosti broja -D na osnovu predstave u komplementu maksimalne vrednosti moguće je izraziti na sledeći način:



% Izvor DOCX: blok 91
\begin{equation}\label{eq:1.2.3.2}
\operatorname{KO}_n(-D)=\bigl(\operatorname{KMV}_n(-D)+1\bigr)\bmod r^n
\end{equation}


% Izvor DOCX: blok 92
Najpre se svaka cifra apsolutne vrednosti zameni cifrom $r-1-d_i$, a zatim se celoj reči doda jedinica. Prenos se propagira od cifre najmanje težine:
...
% Izvor DOCX: blok 94
Skup brojeva koje je moguće zapisati i komplementu osnove, ukoliko je na raspolaganju \emph{n} cifara u osnovi \emph{r}, definisan je sledećom relacijom:
...
% Izvor DOCX: blok 129
Ukoliko u ovu jednačinu zamenimo poznato rešenje x=12=r+2 dobijamo:
...
(101110)_{\mathrm{ZA}}=(1001110)_{\mathrm{ZA}}=-14
\end{equation}
\begin{equation}\label{eq:2.3.11}
(010100)_{\mathrm{ZA}}=(0010100)_{\mathrm{ZA}}=20
\end{equation}
...
\item Koristimo formulu \eqref{eq:1.2.2.1} da bi dobili predstavu negativnog broja
...
\{ - \left( \frac{10^{4}}{2} - 1 \right),\  + \left( \frac{10^{4}}{2} - 1 \right)\}
\end{equation}
...
\ \ \ \  - 1952 \equiv 9999 - 1952 = 8047_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.4}
\ \ \ \  - 4399 \equiv 9999 - 4399 = 5600_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.5}
\ \ \ \  - 34 \equiv 9999 - 34 = 9965_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.6}
\ \ \ \ \ \ \ 0 \equiv 9999 - 0 = 9999_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.7}
-1 \equiv 9999 - 1 = 9998_{KMV}
\end{equation}
...
\ \ \ \  - F24 \equiv FFFF - F24 = {F0DB}_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.13}
\ \ \ \  - 1E \equiv FFFF - 1E = {FFE1}_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.14}
\ \ \ \ \ \ \ 0 \equiv FFFF - 0 = {FFFF}_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.15}
-1 \equiv FFFF - 1 = {FFFE}_{KMV}
\end{equation}
...
\ \ \ \  - 42 \equiv 7777 - 42 = 7735_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.22}
\ \ \ \ \ \ \ 0\  \equiv 7777 - 0 = 7777_{KMV}
\end{equation}
\begin{equation}\label{eq:2.4.23}
-1 \equiv 7777 - 1 = 7776_{KMV}
\end{equation}
...
% Izvor DOCX: blok 195
\begin{enumerate}[label=\alph*),start=1,leftmargin=*]
\item Predstaviti sledeće označene brojeve u komplementu osnove sa 4 cifre:
...
% Izvor DOCX: blok 197
b) Predstaviti sledeće označene brojeve u komplementu maksimalne vrednosti sa 4 cifre:
...
% Izvor DOCX: blok 203
a kao negativan ako je cifra nejveće težine iz skupa:
...
- 45_{10} \equiv 10000 - 0045 = 9955_{10 - KO}
\end{equation}
...
- 54_{10} \equiv 9999 - 9954 = 0045_{10 - KMV}
\end{equation}
...
% Izvor DOCX: blok 233
\textbf{-18}: Pošto je broj dat u osnovi 10 a zahteva se predstava broja u osnovi 2, najpre je neophodno izvršiti konverziju broja, iza znaka „-“, iz jednog u drugi brojni sistem pri čemu se u binarnom brojnom sistem, deo broja iza znaka „-“ predstavlja na traženom broju bita. Dakle, predstava u binarnom brojnom sistemu, na traženom broju bita, je:
...
- \ 00010010 \equiv 11111111 - 00010010 + 1 = 11101110
\end{equation}
...
- \ 01111000 \equiv 11111111 - 01111000 + 1 = 10001000
\end{equation}
...
% Izvor DOCX: blok 248
b) Najpre proširimo znak sa tri na šest bita, a zatim odredimo drugi komplement na šest bita. Time postupak ostaje ispravan i za najmanji trobitni broj $100_2=-4$, čija suprotna vrednost ne staje na tri bita.
