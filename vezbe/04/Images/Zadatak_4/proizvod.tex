...
\begin{document}
\begin{tikzpicture}[circuit logic US,thick,>=Latex]
...
\draw[->](3,1.5)--(7,1.5)node[midway,above]{$D_1=0s_2s_1s_00$};
\draw[->](3,-.5)--(7,-.5)node[midway,below]{$D_3=s_2s_1s_000$};\draw(3,1)--(3,-.5);
\draw[->](4,.2)--(7,.2)node[midway,above]{$D_2=0a_1a_0\bar a_1\bar a_0$};
\draw[->](8.5,-2)node[below]{$B=b_1b_0$}--(8.5,-1);\draw[->](10,1)--(11,1)node[right]{$X[4:0]$};
\node[not gate US,draw] (na1) at (1,-1.2){};
\node[not gate US,draw] (na0) at (1,-2.1){};
\draw(-.5,-1.2)node[left]{$a_1$}--(na1.input);\draw(na1.output)--(2.2,-1.2)node[right]{$\bar a_1$};
\draw(-.5,-2.1)node[left]{$a_0$}--(na0.input);\draw(na0.output)--(2.2,-2.1)node[right]{$\bar a_0$};
