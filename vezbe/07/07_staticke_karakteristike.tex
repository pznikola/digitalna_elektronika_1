...
\end{figure}
Još pregledniji prikaz dobija se preslikavanjem karakteristike u odnosu na pravu $v_I=v_U$ (slika~\ref{fig:1.4}). Za prikazanu karakteristiku preseci van dijagonale određuju par stabilnih nivoa $(V_L,V_H)$ i $(V_H,V_L)$, za koje važi $f(V_L)=V_H$, $f(V_H)=V_L$. Presek na dijagonali određuje prag $V_S=f(V_S)$; za prikazanu karakteristiku on je nestabilan. Oblasti između krive i njenog odraza usmeravaju iteraciju ka stabilnim nivoima, što predstavlja regeneraciju. Sam presek ne dokazuje stabilnost: za gladak dvociklus lokalni uslov je $|f^\prime(V_L)f^\prime(V_H)|<1$, dok je za fiksnu tačku uslov $|f^\prime(V_S)|<1$. Kada je odgovarajući modul veći od 1, tačka je nestabilna; granični slučaj jednakosti zahteva dodatnu analizu.
...
Posmatramo statički poremećaj amplitude $V_N\ge0$ na ulazu samo jednog invertora; prethodni deo lanca već je dostigao stabilne nivoe. Ne razmatramo trajanje impulsa šuma niti vremenski odziv. Ako je prethodni izlaz visok, najnepovoljniji šum ga umanjuje; ako je nizak, uvećava ga. Za očuvanje logičke informacije potrebno je ostati sa odgovarajuće strane praga:
...
Za garantovano prenošenje logičke informacije pri šumu na svakoj međuvezi biramo ulazne intervale koji se preslikavaju u odgovarajuće izlazne intervale i obezbeđuju rezervu za šum. Lokalni uslov slabljenja male perturbacije u jednom stepenu je
\begin{equation}\label{eq:2.3}
\left|\frac{\mathrm d f}{\mathrm d v_U}\right|<1.
\end{equation}

Za uobičajenu glatku invertorsku karakteristiku sa dve krajnje oblasti malog pojačanja, konvencionalne granice $V_{IL},V_{IH}$ određuju se nagibom $-1$. Kod izlomljene karakteristike uzimaju se prelomi na kojima modul nagiba prelazi jedinicu. Za takve izabrane granice dovoljni uslovi očuvanja dozvoljenih intervala su
...
Ovo nisu opšte tačne jednakosti. SSNM opisuje regeneraciju nakon jednog lokalnog statičkog poremećaja; garantovane margine dozvoljenih intervala koriste se pri povezivanju kola. U svim formulama pretpostavlja se da ulazi ostaju u domenu zadate karakteristike; ponašanje izvan opsega napajanja ovde nije zadato. U svim sledećim zadacima prikazane su i tačne vrednosti prema~\eqref{eq:2.7} i procene prema~\eqref{eq:2.8} iz izvornika.
...
\paragraph{b) Regeneracija.}
Prvo kolo privlači unutrašnje tačke sredini; izuzeci su tačne vrednosti $0$ i $5\,\mathrm V$, koje ostaju na nestabilnim krajnjim fiksnim tačkama. Drugo kolo zadržava svaku ulaznu vrednost. Treće kolo obnavlja niski ili visoki nivo, jer je nagib $1/2$ u krajnjim oblastima, a $3$ oko praga. Samo ono je pogodno kao logički bafer.
...
\end{figure}
\subsection*{Rešenje}
Margine određujemo za lanac čiji je jedan stepen cela novodobijena kaskada. Izvori šuma su na međuspojevima takvih stepena; unutrašnja veza invertora i bafera smatra se idealnom. Šum i na toj unutrašnjoj vezi zahteva zaseban model dva poremećena kola.
...
\begin{center}\begin{minipage}{\linewidth}\centering\captionsetup{hypcap=false}\captionof{table}{Prelomne tačke kompozicije invertora i bafera.}\label{tab:prelomi}\begin{tabular}{cccc}\toprule
