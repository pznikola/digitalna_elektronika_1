...
Koristimo pojednostavljeni model izvornika, bez modulacije dužine kanala, struja curenja i parazitskih kapacitivnosti, osim gde su izričito potrebne. Za NMOS označimo $B=\mu_nC_{ox}W/L$, $E=E_C L$ i $u=V_{GS}-V_T$. Za $u\le0$ je $I_D=0$. Za $u>0$ i $V_{DS}\ge0$, struje u omskoj oblasti i zasićenju su
\begin{equation}\label{eq:model}
I_{D,\mathrm{lin}}=\frac{B}{1+V_{DS}/E}\left(uV_{DS}-\frac{V_{DS}^2}{2}\right),\qquad I_{D,\mathrm{sat}}=\frac{B}{2}\frac{u^2}{1+u/E}.
\end{equation}

U ovom modelu granica oblasti uzima se kao $V_{DS}=u$, a $v_{\mathrm{sat}}=\mu_nE_C/2$. To je zadati aproksimativni model, a ne model konkretne tehnologije. Struja je na toj granici neprekidna, ali nagib omskog izraza po $V_{DS}$ postaje negativan pre granice i ne spaja se glatko sa konstantnom strujom zasićenja. Zato ovaj izraz ne daje fizički pouzdanu karakteristiku u celoj okolini prelaska u zasićenje. U nastavku se čuvaju zadate jednačine i posebno označavaju dodatne aproksimacije. Za $V_{DS}/E\ll1$, odnosno $u/E\ll1$, odgovarajući imenilac može se aproksimirati jedinicom. Za PMOS se koriste pozitivni naponi $V_{SG},V_{SD}$ i prag $|V_{TP}|$.
...
\end{figure}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Izlazni nivoi i pragovi.}
...
\FloatBarrier\Needspace{6\baselineskip}\paragraph{b) Numerički rezultati.}
$B_n=860\,\mathrm{\mu A/V^2}$ i $\alpha=0.0581395\,\mathrm V$. U aproksimaciji bez korekcije kratkog kanala:
\begin{center}\begin{minipage}{\linewidth}\centering\captionof{table}{Aproksimativni nivoi invertora sa otpornim opterećenjem.}\label{tab:1.nivoi}\begin{tabular}{lc}\toprule
Veličina & Vrednost\\
\midrule
$V_H$ & $1.2\,\mathrm V$\\
\redlinija
...
\clearpage\section{Zadatak 2: opterećenje sa indukovanim kanalom}\label{sec:2}
\begin{enumerate}[label=\alph*)]
...
\end{figure}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Efekat podloge i odnos geometrija.}
...
\FloatBarrier\Needspace{6\baselineskip}\paragraph{b) Povišeno upravljanje opterećenjem.}
...
\clearpage\section{Zadatak 3: opterećenje sa ugrađenim kanalom}\label{sec:3}
...
\FloatBarrier\Needspace{6\baselineskip}\paragraph{Određivanje $V_{IL}$.}
...
\FloatBarrier\Needspace{6\baselineskip}\paragraph{Određivanje $V_{IH}$ i niskog nivoa.}
...
\clearpage\section{Zadatak 4: CMOS invertor}\label{sec:4}
\begin{enumerate}[label=\alph*)]
...
\end{enumerate}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Oblasti rada.}
...
Suprotne stroge nejednakosti određuju omske oblasti; jednakosti su granice. Isključene oblasti određuju samo uslovi gejta. Na $v_U=v_I=V_S$ oba tranzistora su u zasićenju. Oznake I--V na slici prate karakteristiku prenosa; za proizvoljnu tačku van nje koriste se prethodne nejednakosti. Bez modulacije dužine kanala srednji deo idealizovane karakteristike je vertikalan: pri jednakim strujama zasićenja statičke jednačine ne izdvajaju jedan izlazni napon. Crna linija prikazuje samo redosled oblasti, a brojna vrednost $V_S$ računa se u tački b).
\begin{figure}[!htbp]\centering
...
\end{figure}
\FloatBarrier\Needspace{17\baselineskip}\paragraph{b) Napon prebacivanja.}
...
\begin{center}\begin{minipage}{\linewidth}\centering\captionof{table}{Prag CMOS invertora za dve širine PMOS tranzistora.}\label{tab:4.prag}\begin{tabular}{ccc}\toprule
...
\clearpage\section{Zadatak 5: sinteza statičkog CMOS kola}\label{sec:5}
\begin{enumerate}[label=\alph*)]
...
\end{enumerate}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Raspoloživi komplementarni ulazi.}
...
\end{figure}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{b) Samo direktni ulazi.}
Prvo se realizuje $Y_1=\overline{(A+B)(C+D)}$ pomoću osam tranzistora, pa se doda izlazni CMOS invertor sa dva tranzistora. Ukupno je potrebno deset tranzistora. Četiri zasebna ulazna invertora uz realizaciju iz a) zahtevala bi šesnaest. Broj deset je minimalan u acikličnim mrežama komplementarnih statičkih CMOS kola sa dualnim redno-paralelnim PDN/PUN mrežama, uz direktne primarne ulaze. Svaki od četiri bitna ulaza mora upravljati bar jednim NMOS-om i odgovarajućim PMOS-om. Jedan takav stepen sa direktnim ulazima daje nerastuću funkciju svakog ulaza, dok je tražena funkcija rastuća, pa je potreban i unutrašnji signal između stepena. On upravlja bar još jednim parom tranzistora: ukupno najmanje $2(4+1)=10$. Prikazana realizacija dostiže ovu granicu.
\begin{figure}[!htbp]\centering
...
\end{figure}
\clearpage\section{Zadatak 6: dualne PDN i PUN mreže}\label{sec:6}
...
\end{figure}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Logička funkcija.}
\begin{equation}\label{eq:6.1}
\bar Y=A+B(C+D),\qquad Y=\overline{A+B(C+D)}=\bar A\bar B+\bar A\bar C\bar D.
\end{equation}

\FloatBarrier\Needspace{6\baselineskip}\paragraph{b) Konstrukcija dualne mreže.}
...
\end{figure}
\clearpage\section{Zadatak 7: dimenzionisanje tranzistora}\label{sec:7}
...
\end{figure}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
Sama pokretljivost ne određuje apsolutne dimenzije. Kao u izvorniku, uzimamo zajedničku dužinu kanala, referentni invertor sa istim oksidom, približno jednakim apsolutnim pragovima i normalizovanim odnosima $(W/L)_n=1$, $(W/L)_p=2$ i približno izjednačavamo najgore otpornosti provodnih putanja. U nastavku $w$ označava normalizovani odnos $W/L$.
...
\end{figure}
\clearpage\section{Zadatak 8: dinamička i domino logika}\label{sec:8}
\begin{enumerate}[label=\alph*)]
...
\end{enumerate}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Svođenje funkcije.}
...
\end{figure}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{b) Domino realizacija.}
...
\end{figure}
\clearpage\section{Zadatak 9: vremenski dijagram dinamičkog kola}\label{sec:9}
...
\end{figure}
\clearpage\section{Zadatak 10: transmisioni gejtovi}\label{sec:10}
\begin{enumerate}[label=\alph*)]
...
\end{figure}
\FloatBarrier\Needspace{10\baselineskip}\subsection*{Rešenje}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{a) Binarni ulazi.}
...
\end{equation}
\begin{center}\begin{minipage}{\linewidth}\centering\captionof{table}{Funkcionalna tabela za binarne ulaze.}\label{tab:10.1}\begin{tabular}{cc}\toprule
$ABCD$ & $Y$\\
\midrule
\texttt{0000} & 0\\
\redlinija
...
\FloatBarrier\Needspace{6\baselineskip}\paragraph{b) Visoka impedansa na ulazu D.}
...
\begin{center}\begin{minipage}{\linewidth}\centering\captionof{table}{Izlaz kada D može biti i visoke impedanse.}\label{tab:10.2}\begin{tabular}{cc}\toprule
$ABC$ & $Y$\\
\midrule
000 & 0\\
\redlinija
...
111 & 1\\
\bottomrule\end{tabular}\end{minipage}\end{center}
\FloatBarrier\Needspace{6\baselineskip}\paragraph{c) Sinteza funkcije.}
